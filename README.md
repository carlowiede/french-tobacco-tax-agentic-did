...
```bash
git clone https://github.com/carlowiede/french-tobacco-tax-agentic-did.git
cd french-tobacco-tax-agentic-did
```

No Terminal experience? Click the green **"Code"** button on the GitHub page and select **"Download ZIP"**. Unzip it and note the folder path.

### Step 2 — Obtain SHARE data

This repository does not contain SHARE microdata. Request access at:

> https://share-eric.eu/data/data-access

Once granted, download the following modules for waves 1, 2, 4, 5, 6, 8, 9 from SHARE release 9.0.0:

- `br` — Behavioural risks (smoking: `br002_`)
- `co` — Consumption (income proxy: `co007_`)
- `dn` — Demographics (age, sex, education)
- `cv_r` — Cover screen (identifiers)
- `gv_isced` — Harmonised education

Place the downloaded `.dta` files in a subfolder called `SHARE WAVES/` inside your cloned repository folder.

### Step 3 — Set the root path

Open `run_all.do` and change the following line to match your local folder path:

```stata
global root "/Users/carlo/MIB/Master Thesis/Latest_Run"  // <-- CHANGE THIS
```

For example:
```stata
* Mac
global root "~/repos/french-tobacco-tax-agentic-did"

* Windows
global root "C:/Users/yourname/french-tobacco-tax-agentic-did"
```

> **Important:** If you forget to update this line, Stata will stop immediately and display an error message telling you exactly what to change. No results will be produced until the path is correct.

### Step 4 — Run the pipeline

Open `run_all.do` in Stata and run it. This executes the full pipeline in sequence, installs any missing packages automatically, and saves a timestamped log to the `output/` folder.

All tables and figures in the thesis are produced by the do-files in `do-files/`. The `results_summary.md` file reports the expected output for every specification — use it to verify your results match.
